...
// user buttons
//  var camMode = false; ->scrapped, a first person mode?
var f1cam = false;
var f2cam = false;
var f3cam = false;
var f4cam = false;
...
// shapeslist
const shapes = [
  //drawCube(geometry, 0x44aa88,  0),
  //drawCube(cube, 0x8844aa, -4, 0, 0),
  //drawCube(cube, 0xaa8844,  4, 0, 0),
  //drawCube(cube, 0xaa8844,  light.position.x, light.position.y, light.position.z), 
];
...
// boat - 17, 18, 19 - was gonna add a boat game n everything ;(
//const boatip = new THREE.CylinderGeometry(0, 3, 10, 3 );
//const boatfront = new THREE.Mesh( boatip, )
// fisher - 20, 21
// pole - 22, 23
...
function render(time) {
  ...
  dcube.position.y = 1.5*Math.pow(Math.cos(2 * Math.sin(time)), 2)*5-8;//Math.cos(time*2)*5 - 8;
...
/*  //floating shi around cube
  dcube2.position.x = Math.sin(time)*10;
  dcube2.position.y = Math.cos(time) + 2.5;
  dcube2.position.z = Math.sin(time*0.5)*10;

  dcube3.position.x = Math.sin(time*0.5)*10;
  dcube3.position.y = Math.cos(time) + 3.5;
  dcube3.position.z = Math.sin(time+.2)*10;

  //dcube3.position.x = Math.tan(time)*10;
  //dcube3.position.y = Math.tan(time) + 3.5;
  //dcube3.position.z = Math.tan(time)*10;
*/
...
function drawFish(color) {
  ...
  const fbody = new THREE.BoxGeometry( 0.4, 0.25, 0.05 );
  const fb = new THREE.Mesh( fbody, color );
  fb.position.z = 10;
  scene.add( fb );
  const ftail = new THREE.BoxGeometry( 0.3, 0.30, 0.05 );
  const ft = new THREE.Mesh( ftail, color );
  fb.add( ft );
  ft.position.x += 0.35;
...
function drawCube(geometry, color, x, y, z) {
  const material = new THREE.MeshPhongMaterial({color});
